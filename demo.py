...
def main(args):
    ...
    for image_path in tqdm(images_list):
        ...
        if args.save_glb and len(outputs) > 0:
            ...
            for idx, glb_output in enumerate(outputs):
                glb_path = os.path.join(
                    output_folder,
                    f"{base_name}_person_{idx:03d}.glb",
                )
                ...
                coord_scale = 1.0
                joint_positions = (
                    glb_output["pred_joint_coords"].astype(np.float32)
                    * coord_fix
                    * coord_scale
                )
                joints = []
                for joint_index, joint_name in enumerate(
                    character.skeleton.joint_names
                ):
                    ...
                    if joint_parent == -1:
                        ...
                        offset = (
                            joint_positions[joint_index] - joint_positions[joint_parent]
                        )
                    joints.append(
                        pym_geometry.Joint(
                            joint_name,
                ...
                posed_mesh = pym_geometry.Mesh(
                    glb_output["pred_vertices"].astype(np.float32)
                    * coord_fix
                    * coord_scale,
                ...
                baked_character = pym_geometry.Character(
                    ...
                    posed_mesh, character.skin_weights
                ).rebind_skin()
